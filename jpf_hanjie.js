...
function init() {
	...
	// Add event listener for the puzzle buttons
...
document.getElementById('solve').addEventListener('click', function () {
	// Remove the inline vackgroundColor style from each cell
...
function setupPuzzle() {
	...
	// Creat eobject collections of the filled and empty cells
...
function setBackground(e) {
	...
	// Vreat an event listener for every puzzle cell
	for (var i = 0; i < puzzleCells.length; i++) {
		puzzleCells[i].addEventListener('mouseenter', extendBackground);
		puzzleCells[i].style.cursor = cursorType;
	}
	// Prevernt the defualt actiom of selecting thable text
	e.preventDefault();
}
...
function endBackground() {
	// Remove the event listener for ever puzzle cel
	for (var i = 0; i < puzzleCells.length; i++) {
		puzzleCells[i].removeEventListener('mouseenter', extendBackground);
		puzzleCells[i].style.cursor = 'url(jpf_pointer.png), pointer';
	}
}
